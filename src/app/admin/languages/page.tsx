...
import { mockLanguages, filterLanguages } from "@/data/languages";
...
export default function LanguagesPage() {
  ...
  const [filters, setFilters] = useState({
    ...
    isDefault: "" as "" | "default" | "non-default",
    isRTL: "" as "" | "rtl" | "ltr",
  });
  ...
  const handleEdit = (language: Language) => {
    console.log("Edit language:", language.id);
  };
  ...
  const filteredLanguages = filterLanguages(mockLanguages, searchTerm, filters);
    ...
    <div className="py-6 space-y-6 animate-fadeIn">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className={`mt-1 md:mt-2 text-xs md:text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
              Manage all supported languages for the platform
            </p>
          </div>
              ...
              {showFilterDropdown && (
                <div className="absolute right-0 top-full mt-2 z-50 w-64 rounded-lg border shadow-lg p-4 space-y-3"
                  ...
                  <div>
                    ...
                    <select
                      value={filters.isRTL}
                      onChange={(e) => handleFilterChange("isRTL", e.target.value)}
                      className={`w-full px-3 py-2 rounded-lg border text-sm ${
                        isDarkMode
                          ? "bg-gray-800 border-gray-700 text-white"
                          : "bg-white border-gray-300 text-gray-900"
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className={`divide-y ${isDarkMode ? "divide-gray-800" : "divide-gray-200"}`}>
              {paginatedLanguages.map((language, idx) => {
                const isExpanded = expandedRows.has(language.id);
                  ...
                  <Fragment key={language.id}>
                    ...
                    >
                      ...
                      <td className={`px-4 py-4 ${isDarkMode ? "text-white" : "text-gray-900"}`}>
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-2xl ${
                            ...
                            {language.flag || <Globe className="w-5 h-5 text-[#FF5A22]" />}
                          </div>
                          <div>
                      ...
                      <td className={`hidden md:table-cell px-4 py-4 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                        <span className="font-mono text-sm font-medium">{language.code}</span>
                      </td>
                            ...
                            >
                              <div className={`p-4 rounded-lg border ${isDarkMode ? "bg-gray-800/50 border-gray-700" : "bg-white border-gray-200"}`}>
                                ...
                                <div className={`text-sm space-y-1 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                                  ...
                                  <div><span className="font-medium">Native:</span> {language.nativeName}</div>
                              ...
                              <div className={`p-4 rounded-lg border ${isDarkMode ? "bg-gray-800/50 border-gray-700" : "bg-white border-gray-200"}`}>
                                ...
                                <div className={`text-sm space-y-1 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                                  <div><span className="font-medium">Direction:</span> {language.isRTL ? "Right-to-Left" : "Left-to-Right"}</div>
                                  <div><span className="font-medium">Status:</span> {language.isActive ? "Active" : "Inactive"}</div>
                                  <div><span className="font-medium">Default:</span> {language.isDefault ? "Yes" : "No"}</div>
                                </div>
                              </div>
                              ...
                              <div className={`p-4 rounded-lg border ${isDarkMode ? "bg-gray-800/50 border-gray-700" : "bg-white border-gray-200"}`}>
                                ...
                                <div className={`text-sm space-y-1 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                                  <div><span className="font-medium">Created:</span> {formatDate(language.createdAt)}</div>
                                  <div><span className="font-medium">Updated:</span> {formatDate(language.updatedAt)}</div>
                                </div>
                              </div>
                            </motion.div>
                          </td>
      ...
      {filteredLanguages.length === 0 && (
        <div className={`rounded-lg border p-12 text-center ${isDarkMode ? "bg-gray-900 border-gray-800" : "bg-white border-gray-200"}`}>
          <Globe className={`w-12 h-12 mx-auto mb-4 ${isDarkMode ? "text-gray-600" : "text-gray-400"}`} />
          <h3 className={`text-lg font-semibold mb-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}>
            No languages found
          </h3>
          <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
            {searchTerm ? "Try adjusting your search criteria" : "No languages have been added yet"}
          </p>
        </div>
      )}
    </div>
