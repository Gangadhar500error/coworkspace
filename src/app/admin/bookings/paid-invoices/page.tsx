/**
 * Admin Paid Invoices Page
 * 
 * Displays all paid invoices (bookings with paid status and invoice generated).
 * 
 * DATA SOURCE:
...
function PaidInvoicesPageContent() {
  ...
  // Apply filters - only show paid invoices
  const applyFilters = (bookings: Booking[]) => {
    // Filter for paid invoices only
    let filtered = bookings.filter(
      (booking) => booking.payment.status === "paid" && booking.invoiceGenerated === true
    );
  ...
  const clearFilters = () => {
    ...
    if (customerIdFromUrl) {
      router.push("/admin/bookings/paid-invoices");
    }
  };
  ...
  const totalRevenue = filteredInvoices.reduce((sum, invoice) => sum + invoice.billing.total, 0);
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div>
            ...
            <p className={`mt-1 md:mt-2 text-xs md:text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
              View all paid invoices across the platform
              {filters.customerId && (
              ...
              {paginatedInvoices.length === 0 ? (
                <tr>
                  <td colSpan={8} className={`px-4 py-12 text-center ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
                    <p className="text-sm font-medium">No paid invoices found</p>
                    ...
                    <Fragment key={invoice.id}>
                      ...
                      >
                        ...
                        <td className={`hidden lg:table-cell px-4 py-4 text-center ${isDarkMode ? "text-white" : "text-gray-900"}`}>
                          <div className="font-bold text-sm text-green-600">{formatPrice(invoice.billing.total, invoice.billing.currency)}</div>
                        </td>

                        {/* Paid Date - Hidden on mobile */}
                        <td className={`hidden md:table-cell px-4 py-4 text-center ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    {invoice.payment.paidAt && (
                                      <div className="flex items-center gap-2">
                                        <Calendar className={`w-4 h-4 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`} />
                                        <span className={`text-sm ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                                          Paid: {formatDateTime(invoice.payment.paidAt)}
                                        </span>
                                      </div>
                                    )}
                                  </div>
